...
class ResBlock1D(nn.Module):
    def __init__(self, in_channels, out_channels, use_dsconv=False):
        super(ResBlock1D, self).__init__()
        self.use_dsconv = use_dsconv
        if use_dsconv:
            # 使用蛇形卷积替换标准卷积
            self.conv1 = DSConv(in_channels=in_channels, out_channels=out_channels, kernel_size=3, ...)
            self.conv2 = DSConv(in_channels=out_channels, out_channels=out_channels, kernel_size=3, ...)
        else:
