...
@REGISTRY.register('snake_resnet')
class SnakeResNetModel(nn.Module):
    def __init__(self, num_classes=5, classify=True, use_snake_conv=True):
        super(SnakeResNetModel, self).__init__()
        self.use_snake_conv = use_snake_conv
        self.resnet = models.resnet18(pretrained=False)

        if self.use_snake_conv:
            # 替换第一个卷积层为蛇形卷积层
            self.resnet.layer1[0].conv1 = DSConv(
                in_ch=64,  # 输入通道数
                out_ch=64,  # 输出通道数
                kernel_size=15,  # 核大小
                extend_scope=1,
