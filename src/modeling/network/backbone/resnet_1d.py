...
class ResNet1D(nn.Module):
    def __init__(self, num_classes=1, classify=True):
        ...
        #     nn.Conv1d(in_channels=1, out_channels=128, kernel_size=3, stride=2),  # 调整步长为2
        #     nn.BatchNorm1d(num_features=128),
        #     nn.LeakyReLU(),
        #     ResBlock1D(in_channels=128, out_channels=128),
        #     nn.MaxPool1d(kernel_size=2, stride=2),  # 调整 kernel_size 和 stride 为2
        #     ResBlock1D(in_channels=128, out_channels=128),
        #     # 移除了一些 MaxPool1d 层以防止过度减小尺寸
